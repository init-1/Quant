classdef RiskEMA < RiskModel
    methods
        function o = RiskEMA(id, varargin)
        % Usage:
        %     o = RiskEMA(id, owner);
        %     o = RiskEMA(id, dates, aggid, isLive);
        % where
    ...
    methods(Access = protected)
        ...
        % faccov, exposure, specrisk, beta
            ...
            nFac = length(facflds);
            flds = [facflds; 'SpecificRisk'; 'Beta'; 'R2'];
            nFld = length(flds);
            ...
            for t = 1:length(o.dates)
                ...
                EMExp = DB('QuantTrading').runSql('axioma.GetEMRiskExp_Full', tstr, aggid(2:end),o.owner.isLive);
