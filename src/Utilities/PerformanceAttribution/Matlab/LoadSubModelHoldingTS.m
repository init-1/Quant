function [pffts, bmfts, actfts] = LoadSubModelHoldingTS(StrategyId, ModelId, SubModelId, islive, aggID, dtStart, dtEnd, DBServer, varargin)
% FUNCTION: LoadModelHoldingTS
...
%   ModelId - The modelid of the strategy
...
if(~exist('DBServer', 'var'))
	DBServer = gbl.DBServer;
end

% if ismember({StrategyId}, {'FAC_MSHC','FAC_MSWO','FAC_EAFE','FAC_APHD','FAC_APHD_TW'})
%     isLive = 0;
% else
%     isLive = 1;
% end

sqlRet = runSP('quantworkspace','rpw.usp_getSubModelActHoldingTS',{StrategyId, ModelId, SubModelId, dtStart, dtEnd, aggID, islive}, DBServer);
