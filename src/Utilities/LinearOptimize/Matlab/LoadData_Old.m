...
function LoadData_Old(filename, signal, aggid, startdate, enddate, freq, lqaggid, startpf)
...
try 
    ...
    disp('Error occured when retrieving the daily volume: backtest will not have liquidity constraints using this data set');
    disp(['Error details: ', getReport(exception)]);
    clear ('volume','adv');
end
...
% align data 
if exist('adv','var')
    [bmhd, signal, fwdret, gics, price, adv] = aligndata(bmhd, signal, fwdret, gics, price, adv, 'union');
else
    [bmhd, signal, fwdret, gics, price] = aligndata(bmhd, signal, fwdret, gics, price, 'union');
end
